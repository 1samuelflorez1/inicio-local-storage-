...
// localStorage.setItem('dataCitas', dataCitas) //Sin serializar



// let dataCitasJSON = JSON.stringify(dataCitas) //Con serializar
// localStorage.setItem('dataCitasJSON', dataCitasJSON)

//verifica si ya hay citas en local storage si no las almacena serializandolas 

if (!localStorage.getItem('Citas')){
    localStorage.setItem('Citas', JSON.stringify(dataCitas))  //esto funciona para crear de forma serializada si no es existente
}

//Recupera las citas del local storage y las deserielizadas y retorna las citas obtenidas 

const traeCitas = JSON.parse(localStorage.getItem('Citas'))   //
console.log(traeCitas)

boton.addEventListener('click', mostrarCita)
...
function mostrarCita(){
    ...
    const index = Math.floor(Math.random() * traeCitas.length)
    miCita.innerText = traeCitas[index].cita
    miAutor.innerText = traeCitas[index].autor
